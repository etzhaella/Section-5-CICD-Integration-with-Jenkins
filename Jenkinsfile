pipeline {
    ...
    environment {
        // Your GitHub repo URL and branch
        GIT_REPO_URL = 'https://github.com/etzhaella/Section-5-CICD-Integration-with-Jenkins'
        GIT_BRANCH   = 'main'

        // Docker Hub image (must be lowercase)
        IMAGE_NAME   = 'section5-flask-aws-monitor'
    }
    ...
    stages {
        ...
        stage('Parallel Checks') {
            parallel {
                ...
                stage('Linting') {
                    steps {
                        // MOCK commands – allowed by the exam
                        sh '''
        ...
        stage('Push to Docker Hub') {
            steps {
                // dockerhub-creds = username + token stored in Jenkins Credentials
